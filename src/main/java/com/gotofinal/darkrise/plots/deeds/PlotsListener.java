...
import com.gotofinal.darkrise.plots.util.MaterialUtil;
...
public class PlotsListener implements Listener
{
    ...
    public void onSignClick(final PlayerInteractEvent event)
    {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK)
        {
            ...
            if (plot.isOwned())
            { // Already owned/ Extending rent time
    ...
    public void onSignBreak(final BlockBreakEvent event)
    {
        ...
        if (player.hasPermission("pmco.deeds.build"))
        {
            return;
        }
        final PlotManager mgr = this.plugin.getGlobalPlotsManager().getPlotManager(player.getWorld());
    ...
    public void onBlockPlace(final BlockPlaceEvent event)
    {
        ...
        final ApplicableRegionSet regions = regionManager.getApplicableRegions(block.getLocation());
     ...
     *         player to add
     * @param plot
     *         plot to assign to the {@code player}
     */
