...
import com.promcteam.homestead.events.PlotUpdateEvent;
...
public class PlotsListener implements Listener {
    ...
    public PlotsListener(final Homestead instance) {
            ...
            if (plot.isOwned()) { // Already owned/ Extending rent time
                if (!plot.getOwner().equalsIgnoreCase(player.getName())) {
                    ...
                    if ((item != null) && item.isSimilar(plot.getDeed().toItemStack())) {
                        ...
                        final long expiry =
                                TimeUnit.MILLISECONDS.toHours(plot.getExpiry() - System.currentTimeMillis());
    ...
    public void onSignBreak(final BlockBreakEvent event) {
        ...
        if (!event.getBlock().getType().toString().contains("SIGN") || player.hasPermission("homestead.deeds.build"))
            return;
    ...
    public void onBlockPhysics(final BlockPhysicsEvent event) {
        ...
        mgr.getPlots()
                .values()
                .stream()
                .filter(plots -> plots.getSignLocation().equals(sign.getLocation()))
                .findFirst()
...
//    }
    ...
    public void cancelWarmup(PlayerMoveEvent event) {
        ...
        if (!from.equals(to)) {
            if (Homestead.getInstance().getConfigHandler().IS_BUNGEE) {
                BungeeUtil.sendMessage(BungeeUtil.CHANNEL,
                        event.getPlayer(),
                        "CANCEL_HOME",
        ...
        for (int x = min.getBlockX(); x <= max.getBlockX(); x++) {
            for (int y = min.getBlockY(); y <= max.getBlockY(); y++) {
                for (int z = min.getBlockZ(); z <= max.getBlockZ(); z++) {
                    ...
                    if (block.getType().equals(data.getItemType()) && ((data.getData() == -1) || (block.getData()
                            == data.getData()))) {
                        counter++;
                    }
                }
    ...
    public String toString() {
        return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).appendSuper(super.toString())
                .append("confirmingPlayers", this.confirmingPlayers)
                .append("cooldown", this.cooldown)
                .append("cooldownPlayers", this.cooldownPlayers)
                .toString();
    }
}
