...
import com.promcteam.homestead.config.CustomYaml;
import com.promcteam.homestead.util.ConfigUtil;
...
public class GlobalPlotsManager {
    ...
    public GlobalPlotsManager(final Homestead instance) {
     ...
     *
     * @param typeName name of the Deed to load
    ...
    public Deed loadDeedType(ConfigurationSection cs, String typeName) {
        ...
        if (this.isValidType(typeName)) {
            this.plugin.getLogger()
                    .severe("Tried to load a Plot Type called '" + typeName + "' but there "
                            + "already is one by that name.");
            return null;
        }
        ...
        type.setDisplayName(ChatColor.translateAlternateColorCodes('&',
                ConfigUtil.getOrSet(cs, "display-name", typeName)));
...
//        }
     ...
     *
     * @param world world to get the DeedManager from
    ...
    /**
     * Attempts to find a plot for the given player across all worlds in the server.
     *
     * @param player The player to search for
     * @return a {@link Plot} object or null
     */
    public Plot getPlot(final Player player) {
        ...
        for (World w : Bukkit.getWorlds()) {
            if (pm.getPlotManager(w) == null)
                continue;
            for (final Plot plot : pm.getPlotManager(w).getPlots().values()) {
    ...
    /**
     * Attempts to find a plot by the given name across all worlds in the server.
     *
    ...
    public Plot getPlot(String name) {
        for (World w : Bukkit.getWorlds()) {
            if (getPlotManager(w) == null)
                continue;
            if (getPlotManager(w).getPlot(name) != null)
                return getPlotManager(w).getPlot(name);
        }
    ...
    public ArrayList<Plot> getAllPlots() {
        ...
        for (World w : Bukkit.getWorlds()) {
            if (pm.getPlotManager(w) == null)
                continue;
            for (Map.Entry<String, Plot> entry : getPlotManager(w).getPlots().entrySet()) {
                pts.add(entry.getValue());
            }
        }
        return pts;
    }
     ...
     *
     * @param name name of the Deed to get
     */
    ...
    public String toString() {
        return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).appendSuper(super.toString())
                .append("deedsConfig", this.deedsConfig)
                .append("managers", this.managers)
